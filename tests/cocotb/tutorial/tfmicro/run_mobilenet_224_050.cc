...
#include "tests/cocotb/tutorial/tfmicro/mobilenet_v1_050_224_quant.h"
...
int main(int argc, char** argv) {
  ...
  debug_model_addr = (uint32_t)g_mobilenet_v1_050_224_quant_model_data;

  // -----------------------------------------------------------
  // Step 1: Magic Check
  // -----------------------------------------------------------
  debug_stage = STAGE_MAGIC_CHECK;
  const uint8_t* raw_data = g_mobilenet_v1_050_224_quant_model_data;
  ...
  const tflite::Model* model = tflite::GetModel(g_mobilenet_v1_050_224_quant_model_data);
