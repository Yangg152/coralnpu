...
#include "tests/efficientnet/efficientnet_int8.h"

using MobilenetOpResolver = tflite::MicroMutableOpResolver<20>;
...
TfLiteStatus RegisterOps(MobilenetOpResolver& op_resolver) {
  ...
  // 注册 EfficientNet-Lite 专有/常用算子
  // TF_LITE_ENSURE_STATUS(op_resolver.AddPad());            // Conv 边缘填充
  // TF_LITE_ENSURE_STATUS(op_resolver.AddMaxPool2D());      // 池化
  // TF_LITE_ENSURE_STATUS(op_resolver.AddConcatenation());  // 特征拼接
  // TF_LITE_ENSURE_STATUS(op_resolver.AddRelu6());          // 激活函数
  // TF_LITE_ENSURE_STATUS(op_resolver.AddLogistic());       // 也就是 Sigmoid
  // TF_LITE_ENSURE_STATUS(op_resolver.AddStridedSlice());   // 部分网络切片用
...
int main(int argc, char** argv) {
  ...
  debug_model_addr = (uint32_t)g_efficientnet_int8_model_data;

  // -----------------------------------------------------------
  // Step 1: Magic Check
  // -----------------------------------------------------------
  debug_stage = STAGE_MAGIC_CHECK;
  const uint8_t* raw_data = g_efficientnet_int8_model_data;
  ...
  const tflite::Model* model = tflite::GetModel(g_efficientnet_int8_model_data);
