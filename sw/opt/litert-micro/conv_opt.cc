...
// 权重重排 + IC展开 的小OC优化
// 权重预处理：[OC, FH, FW, IC] → [FH, FW, IC, OC]
// 内层用连续 vle8 替代 strided vlse8

void ConvSmallOC_ReorderWeight(
  ...
  const int kernel_size = filter_height * filter_width * input_depth;

  // =========================================================
  // 权重重排: [OC, FH, FW, IC] → [FH, FW, IC, OC]
  // =========================================================
  auto reordered_filter = make_aligned_array<int8_t>(16, kernel_size * output_depth);
  if (!reordered_filter) return;
  for (int oc = 0; oc < output_depth; ++oc) {
    const int8_t* src = filter_data + oc * kernel_size;
    for (int i = 0; i < kernel_size; ++i) {
      reordered_filter[i * output_depth + oc] = src[i];
    }
  }
  ...
  const int8_t zero_point_val = (int8_t)(-input_offset);
  auto zero_buffer = make_aligned_array<int8_t>(16, input_depth);
  if (!zero_buffer) return;
  std::memset(zero_buffer.get(), zero_point_val, input_depth);
  const int8_t* zero_ptr = zero_buffer.get();

  // OC 很小，一次 vl 覆盖全部
  const size_t vl = __riscv_vsetvl_e32m4(output_depth);

  // 量化参数只加载一次
  vint32m4_t v_mult = __riscv_vle32_v_i32m4(output_multiplier, vl);
  vuint32m4_t v_lshift = __riscv_vzext_vf4_u32m4(
      __riscv_vle8_v_u8m1(lshift_data.get(), vl), vl);
  vuint32m4_t v_rshift = __riscv_vzext_vf4_u32m4(
      __riscv_vle8_v_u8m1(rshift_data.get(), vl), vl);
  vint32m4_t bias_vec = bias_data
      ? __riscv_vle32_v_i32m4(bias_data, vl)
  ...
  for (int b = 0; b < batches; ++b) {
    for (int out_y = 0; out_y < output_height; ++out_y) {
      ...
      int8_t* out_row = output_data +
          ((b * output_height + out_y) * output_width) * output_depth;
      ...
      for (; out_x <= output_width - 4; out_x += 4) {
        ...
        const int in_x_origin0 = (out_x + 0) * stride_w - pad_w;
        const int in_x_origin1 = (out_x + 1) * stride_w - pad_w;
        const int in_x_origin2 = (out_x + 2) * stride_w - pad_w;
        const int in_x_origin3 = (out_x + 3) * stride_w - pad_w;

        for (int ky = 0; ky < filter_height; ++ky) {
          const int in_y = in_y_origin + ky * dilation_h;
          const bool y_valid = (in_y >= 0 && in_y < input_height);
          const int8_t* in_row_base = y_valid
              ? input_data + ((b * input_height + in_y) * input_width) * input_depth
              : nullptr;
          ...
          for (int kx = 0; kx < filter_width; ++kx) {
            ...
            if (!y_valid) {
              p0 = p1 = p2 = p3 = zero_ptr;
            } else {
              int ix0 = in_x_origin0 + kx * dilation_w;
              int ix1 = in_x_origin1 + kx * dilation_w;
              int ix2 = in_x_origin2 + kx * dilation_w;
              int ix3 = in_x_origin3 + kx * dilation_w;
              p0 = (ix0 >= 0 && ix0 < input_width) ? in_row_base + ix0 * input_depth : zero_ptr;
              p1 = (ix1 >= 0 && ix1 < input_width) ? in_row_base + ix1 * input_depth : zero_ptr;
              p2 = (ix2 >= 0 && ix2 < input_width) ? in_row_base + ix2 * input_depth : zero_ptr;
              p3 = (ix3 >= 0 && ix3 < input_width) ? in_row_base + ix3 * input_depth : zero_ptr;
            }

            // 重排后的权重基地址：连续存储
            const int8_t* w_base = reordered_filter.get() +
                (ky * filter_width + kx) * input_depth * output_depth;

            // IC=0
            {
              vint8m1_t w8 = __riscv_vle8_v_i8m1(w_base + 0 * output_depth, vl);
              vint16m2_t w16 = __riscv_vsext_vf2_i16m2(w8, vl);
              acc0 = __riscv_vwmacc_vx_i32m4(acc0, (int16_t)p0[0] + (int16_t)input_offset, w16, vl);
              acc1 = __riscv_vwmacc_vx_i32m4(acc1, (int16_t)p1[0] + (int16_t)input_offset, w16, vl);
              acc2 = __riscv_vwmacc_vx_i32m4(acc2, (int16_t)p2[0] + (int16_t)input_offset, w16, vl);
              acc3 = __riscv_vwmacc_vx_i32m4(acc3, (int16_t)p3[0] + (int16_t)input_offset, w16, vl);
            }
            // IC=1
            {
              vint8m1_t w8 = __riscv_vle8_v_i8m1(w_base + 1 * output_depth, vl);
              vint16m2_t w16 = __riscv_vsext_vf2_i16m2(w8, vl);
              acc0 = __riscv_vwmacc_vx_i32m4(acc0, (int16_t)p0[1] + (int16_t)input_offset, w16, vl);
              acc1 = __riscv_vwmacc_vx_i32m4(acc1, (int16_t)p1[1] + (int16_t)input_offset, w16, vl);
              acc2 = __riscv_vwmacc_vx_i32m4(acc2, (int16_t)p2[1] + (int16_t)input_offset, w16, vl);
              acc3 = __riscv_vwmacc_vx_i32m4(acc3, (int16_t)p3[1] + (int16_t)input_offset, w16, vl);
            }
            // IC=2
            {
              vint8m1_t w8 = __riscv_vle8_v_i8m1(w_base + 2 * output_depth, vl);
              vint16m2_t w16 = __riscv_vsext_vf2_i16m2(w8, vl);
              acc0 = __riscv_vwmacc_vx_i32m4(acc0, (int16_t)p0[2] + (int16_t)input_offset, w16, vl);
              acc1 = __riscv_vwmacc_vx_i32m4(acc1, (int16_t)p1[2] + (int16_t)input_offset, w16, vl);
              acc2 = __riscv_vwmacc_vx_i32m4(acc2, (int16_t)p2[2] + (int16_t)input_offset, w16, vl);
              acc3 = __riscv_vwmacc_vx_i32m4(acc3, (int16_t)p3[2] + (int16_t)input_offset, w16, vl);
            }
          }
        }
        ...
        __riscv_vse8_v_i8m1(out_ptr + 0 * output_depth,
            QuantizeResult_m4(acc0, v_mult, v_lshift, v_rshift,
                output_offset, output_min, output_max, vl), vl);
        __riscv_vse8_v_i8m1(out_ptr + 1 * output_depth,
            QuantizeResult_m4(acc1, v_mult, v_lshift, v_rshift,
                output_offset, output_min, output_max, vl), vl);
        __riscv_vse8_v_i8m1(out_ptr + 2 * output_depth,
            QuantizeResult_m4(acc2, v_mult, v_lshift, v_rshift,
                output_offset, output_min, output_max, vl), vl);
        __riscv_vse8_v_i8m1(out_ptr + 3 * output_depth,
            QuantizeResult_m4(acc3, v_mult, v_lshift, v_rshift,
                output_offset, output_min, output_max, vl), vl);
      }
      ...
      for (; out_x < output_width; ++out_x) {
        ...
        for (int ky = 0; ky < filter_height; ++ky) {
          ...
          for (int kx = 0; kx < filter_width; ++kx) {
            const int in_x = in_x_origin + kx * dilation_w;
            const int8_t* ptr;
            if (y_valid && in_x >= 0 && in_x < input_width) {
              ptr = in_row_base + in_x * input_depth;
            } else {
              ptr = zero_ptr;
            }
            ...
            for (int ic = 0; ic < input_depth; ++ic) {
              vint8m1_t w8 = __riscv_vle8_v_i8m1(w_base + ic * output_depth, vl);
              vint16m2_t w16 = __riscv_vsext_vf2_i16m2(w8, vl);
              int16_t in_val = (int16_t)ptr[ic] + (int16_t)input_offset;
              acc = __riscv_vwmacc_vx_i32m4(acc, in_val, w16, vl);
            }
          }
        }

        __riscv_vse8_v_i8m1(out_row + out_x * output_depth,
            QuantizeResult_m4(acc, v_mult, v_lshift, v_rshift,
                output_offset, output_min, output_max, vl), vl);
      }
    }
  }
...
void ConvPerChannel(
  ...
  if (filter_h == 1 && filter_w == 1 && 
     ...
     Conv1x1PerChannelRVV_Optimized(params, output_multiplier, output_shift,
         input_shape, input_data, filter_shape, filter_data,
         bias_shape, bias_data, output_shape, output_data);
  }
  else if (dilation_h == 1 && dilation_w == 1 && output_depth <= 16) {
     // 小 output_depth：沿空间维度向量化更高效
     ConvSmallOC_ReorderWeight(params, output_multiplier, output_shift,
         input_shape, input_data, filter_shape, filter_data,
         bias_shape, bias_data, output_shape, output_data);
  }
  else if (dilation_h == 1 && dilation_w == 1) {
     ConvGeneralPerChannelRVV_Optimized_Unroll8(params, output_multiplier, output_shift,
         input_shape, input_data, filter_shape, filter_data,
         bias_shape, bias_data, output_shape, output_data);
  }
  else {
